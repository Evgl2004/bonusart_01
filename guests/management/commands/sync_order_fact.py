...
class Command(BaseCommand):
    """
    Синхронизирует таблицу `order_fact` из `olap_sales_raw_line`.
    """

    help = (
        "Пересчитывает order_fact из сырых OLAP-строк. "
    ...
    def _signal_handler(self, signum, frame) -> None:
        ...
        logger.info(
            "Получен сигнал %s, sync_order_fact завершится после текущего прохода.",
            signum,
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS("Запущен sync_order_fact"))
