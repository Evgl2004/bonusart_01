...
class CouponAutoscenarioPreviewTests(TestCase):
    ...
    def test_plan_command_prints_safe_summary(self):
        ...
        self.assertIn("=== Связность сценария ===", output)
        self.assertIn("notification_scenario_active=True", output)
        self.assertIn("template_name=", output)
        self.assertIn("active_bot_profiles=", output)
        self.assertIn("=== Расписание купонных автосценариев ===", output)
        self.assertIn("scenario_in_schedule_codes=", output)
