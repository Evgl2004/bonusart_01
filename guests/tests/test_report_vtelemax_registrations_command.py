...
class ReportVtelemaxRegistrationsCommandTests(TestCase):
    ...
    def test_reports_grouped_rows_by_day_and_platform(self):
        ...
        self._create_channel(platform="telegram", account_created_at=day1, registered_at=day0)
        self._create_channel(platform="telegram", account_created_at=day0, registered_at=day0)
    ...
    def test_fallbacks_to_account_created_at_when_registered_at_missing(self):
        now = timezone.now()
        day0 = now.replace(hour=11, minute=0, second=0, microsecond=0)
        day1 = day0 - timedelta(days=1)
        self._create_channel(platform="max", account_created_at=day1, registered_at=None)
        ...
        text = out.getvalue()
        self.assertIn(f"{day1.date()} | max | 1 | 1", text)
