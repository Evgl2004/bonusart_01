...
def _queue_autoscenario_assignment_cancellation(
    *,
    ...
    Купон не возвращается в пул напрямую. Возврат произойдёт только после подтверждения
...
def create_autoscenario_dispatch_after_vtelemax_ack(
    *,
    ...
    ставится в очередь. Если включён контур iikoCard, ждём два подтверждения:
