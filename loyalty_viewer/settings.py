...
# Интерактивные сообщения и приём пакетных событий нажатия из vtelemax.
MESSAGE_INTERACTIONS_ENABLED = _env_bool("MESSAGE_INTERACTIONS_ENABLED", False)
...
VTELEMAX_MESSAGE_INTERACTION_CALLBACK_HMAC_SECRET = str(
    os.getenv("VTELEMAX_MESSAGE_INTERACTION_CALLBACK_HMAC_SECRET", "") or ""
).strip() or VTELEMAX_SYNC_HMAC_SECRET
VTELEMAX_MESSAGE_INTERACTION_CALLBACK_REQUIRE_HTTPS = _env_bool(
    "VTELEMAX_MESSAGE_INTERACTION_CALLBACK_REQUIRE_HTTPS",
    True,
)
VTELEMAX_MESSAGE_INTERACTION_CALLBACK_TIMESTAMP_TOLERANCE_SECONDS = _env_int(
    "VTELEMAX_MESSAGE_INTERACTION_CALLBACK_TIMESTAMP_TOLERANCE_SECONDS",
    300,
    min_value=1,
)
VTELEMAX_MESSAGE_INTERACTION_CALLBACK_MAX_BODY_BYTES = _env_int(
    "VTELEMAX_MESSAGE_INTERACTION_CALLBACK_MAX_BODY_BYTES",
    65536,
    min_value=1024,
)
VTELEMAX_MESSAGE_INTERACTION_CALLBACK_RATE_LIMIT_PER_MINUTE = _env_int(
    "VTELEMAX_MESSAGE_INTERACTION_CALLBACK_RATE_LIMIT_PER_MINUTE",
    60,
    min_value=1,
)

# Приём события регистрации гостя из vtelemax для welcome-автосценария.
VTELEMAX_REGISTRATION_CALLBACK_ENABLED = _env_bool("VTELEMAX_REGISTRATION_CALLBACK_ENABLED", False)
VTELEMAX_REGISTRATION_CALLBACK_HMAC_SECRET = str(
    os.getenv("VTELEMAX_REGISTRATION_CALLBACK_HMAC_SECRET", "") or ""
).strip() or VTELEMAX_SYNC_HMAC_SECRET
VTELEMAX_REGISTRATION_CALLBACK_REQUIRE_HTTPS = _env_bool(
    "VTELEMAX_REGISTRATION_CALLBACK_REQUIRE_HTTPS",
    True,
)
VTELEMAX_REGISTRATION_CALLBACK_TIMESTAMP_TOLERANCE_SECONDS = _env_int(
    "VTELEMAX_REGISTRATION_CALLBACK_TIMESTAMP_TOLERANCE_SECONDS",
    300,
    min_value=1,
)
VTELEMAX_REGISTRATION_CALLBACK_MAX_BODY_BYTES = _env_int(
    "VTELEMAX_REGISTRATION_CALLBACK_MAX_BODY_BYTES",
    65536,
    min_value=1024,
)
WELCOME_COUPON_ACCEPT_REGISTRATIONS_FROM = str(
...
WELCOME_COUPON_PROCESSING_BATCH_SIZE = _env_int(
    "WELCOME_COUPON_PROCESSING_BATCH_SIZE",
    100,
    min_value=1,
)
WELCOME_COUPON_PROCESSING_MAX_ATTEMPTS = _env_int(
    "WELCOME_COUPON_PROCESSING_MAX_ATTEMPTS",
    8,
    min_value=1,
)
WELCOME_COUPON_PROCESSING_RETRY_BASE_SECONDS = _env_int(
    "WELCOME_COUPON_PROCESSING_RETRY_BASE_SECONDS",
    30,
    min_value=1,
)
WELCOME_COUPON_PROCESSING_RETRY_MAX_SECONDS = _env_int(
    "WELCOME_COUPON_PROCESSING_RETRY_MAX_SECONDS",
    3600,
    min_value=1,
)
if WELCOME_COUPON_PROCESSING_RETRY_MAX_SECONDS < WELCOME_COUPON_PROCESSING_RETRY_BASE_SECONDS:
    WELCOME_COUPON_PROCESSING_RETRY_MAX_SECONDS = WELCOME_COUPON_PROCESSING_RETRY_BASE_SECONDS
...
COUPON_AUTOSCENARIO_CLOSE_ENABLED = _env_bool("COUPON_AUTOSCENARIO_CLOSE_ENABLED", True)
COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_ENABLED = _env_bool(
    "COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_ENABLED",
    False,
)
COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_CRON = str(
    os.getenv("COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_CRON", "30 5,11 * * *")
    or "30 5,11 * * *"
).strip()
...
def _register_olap_schedule_tasks() -> None:
    ...
    if COUPON_AUTOSCENARIO_CLOSE_ENABLED and COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_ENABLED:
        schedule_map["run_coupon_autoscenario_close"] = {
            "func": "guests.tasks.run_coupon_autoscenario_close_task",
            "schedule_type": "C",
            "cron": COUPON_AUTOSCENARIO_CLOSE_SCHEDULE_CRON,
        }
    else:
