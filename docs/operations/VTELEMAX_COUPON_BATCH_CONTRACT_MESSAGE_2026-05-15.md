...
7. `status_update:canceled` с `meta.release_to_pool=true`: купон убирается у гостя, затем SAGUR после ACK может назначить тот же `series+code` другому гостю.
...
10. Только после ACK от vtelemax SAGUR освобождает купон обратно в пул.
