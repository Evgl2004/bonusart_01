...
На стороне SAGUR sync-gate не даст отправить рассылку, пока vtelemax не вернет ACK по каждому нужному assignment item.
...
- `used` и `expired` убирают купон из активных у гостя, но не освобождают его для повторной выдачи;
- SAGUR освобождает купон обратно в пул только после ACK конкретного `canceled` item с `meta.release_to_pool=true`.
...
5. `status_update:expired`: купон скрывается/помечается истекшим, но не освобождается для повторной выдачи.
6. `status_update:canceled` с `meta.release_to_pool=true`: купон убирается у гостя, затем SAGUR после ACK может назначить тот же `series+code` другому гостю.
7. Повторный `status_update:canceled` по уже снятому купону безопасен и возвращает успешный item-level результат.
8. Ответ без `results[]` или без результата по конкретному `event_id` считается некорректным для SAGUR.
...
8. По другому гостю отменяем reserved-назначение, SAGUR отправляет `status_update:canceled` с `release_to_pool=true`.
9. Только после ACK от vtelemax SAGUR освобождает купон обратно в пул.
10. SAGUR назначает тот же `coupon_series + coupon_code` другому гостю.
...
- сохранить согласованную семантику `canceled`, `used`, `expired`;
