...
### 4) Таблица интересующих категорий (`olap_focus_category`)
Назначение: явно хранить категории, которые важны бизнесу для сегментов/рейтингов.

Поля:
1. `id` (PK)
2. `dish_category` (FK -> `olap_dish_category_dict.id`)
3. `is_enabled`
4. `priority_weight` (вес категории в рейтингах)
5. `tag_code` (технический код, например `meat`, `wine`)
6. `comment`
7. `created_at`, `updated_at`

Индексы/ограничения:
1. уникальность на `dish_category`
2. индекс на `(is_enabled, tag_code)`
...
3. Подтверждена необходимость таблицы «интересующих категорий».
