...
12. Ввести реестр исполнения сценариев `code -> handler` для планового запуска:
    - запуск `schedule`-сценариев выполняется через единый реестр обработчиков;
    - management-команда и периодический task используют реестр, а не прямой вызов конкретного сервиса;
    - добавлены тесты на запуск через реестр и поведение для неизвестного кода.
    Статус: `completed`
13. Перевести webhook-сценарий `balance_changed` на вызов через реестр `code -> handler`:
    - `handle_api_webhook` вызывает реестр webhook-обработчиков;
    - balance-логика исполняется через зарегистрированный handler;
    - добавлены тесты на запуск webhook-сценария через реестр и поведение для неизвестного кода.
    Статус: `completed`
...
3. Добавлен фасад `create_notification_event(...)` с явными `route_*` override:
   приоритет/режим целей/список ботов можно задавать явно в бизнес-вызове,
   при этом сценарий из БД остаётся источником значений по умолчанию.
